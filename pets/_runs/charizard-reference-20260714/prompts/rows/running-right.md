Create one horizontal animation strip for Codex pet `charizard`, state `running-right`.
...
State action: Dragging-right flight loop: fly toward screen-right with wingbeats and a hovering body; legs are tucked and never walk.

State requirements:
- Show directional rightward flight through wingbeats, a hovering torso, tucked legs, and attached tail/flame movement only.
